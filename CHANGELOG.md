...
* `Text.Extract` — URL and email extraction at "social-media quality": follows the rules used by [twitter-text](https://github.com/twitter/twitter-text) (and imitated by Slack, Mastodon, and most auto-link implementations), with full UTS #46 IDNA processing for internationalised domain names. Public API is `Text.Extract.urls/2`, `Text.Extract.emails/2`, and `Text.Extract.all/2`. Each result is a structured map with the original Unicode form, the all-ASCII Punycode form, byte-offset span, and parsed RFC 3986 / RFC 5322 components. Three-phase pipeline: candidate scanner (grapheme-aware regex with twitter-text §3 boundary rules) → structural validator (RFC parsing + per-label IDNA via `Unicode.IDNA.to_ascii/2` + IANA TLD lookup) → boundary cleanup (trailing-punctuation strip + paren balancing). Passes all 103 URL conformance fixtures vendored from twitter-text. Options include `:require_scheme`, `:schemes`, `:tld_mode` (`:iana` / `:any`), `:eai` (RFC 6531 SMTPUTF8 emails), `:strict_idn` (UTR #39 §5.1 single-script restriction + STD3 ASCII rules — defends against `аpple.com`-style homograph attacks), and `:twitter_quirks` (off by default; opts into Twitter's `t.co` 40-char slug rule and English possessive `'s` stripping for behavioural parity with Twitter). Bundled IANA TLD list at `priv/extract/tlds.txt` (~1,440 entries). Depends on the new `:unicode_idna ~> 0.1` package for UTS #46.

* `Text.Extract.split/2` — splits a string into a list of plain-text fragments and entity maps in document order, preserving the original byte-for-byte. The building block for any custom rendering pass: walk the list, leave strings alone, render entities however the caller wants (anchors, mentions, badges, link-preview cards, …).

* `Text.Extract.autolink/2` — convenience wrapper over `split/2` that produces HTML with URLs and emails wrapped in `<a>` anchors. Returns `Phoenix.HTML.safe()` (`{:safe, iodata}`) so it drops straight into Phoenix templates without `raw/1`; non-Phoenix callers convert with `Phoenix.HTML.safe_to_string/1`. Plain-text segments and anchor display text are HTML-escaped via `Phoenix.HTML.html_escape/1`. Anchor display preserves the original Unicode form (e.g. `bücher.de`); the `href` uses Punycode (`xn--bcher-kva.de`) and adds `mailto:` for emails. Schemeless URLs get an explicit scheme in the href (default `https`, override with `:href_scheme`). Options include `:url_attrs`/`:email_attrs` (extra HTML attributes — e.g. `[target: "_blank", rel: "noopener noreferrer"]`) and `:url_renderer`/`:email_renderer` callbacks for full per-entity render control. Custom renderer return values are treated as untrusted strings (HTML-escaped) unless wrapped in `{:safe, iodata}`.

* `mix text.download_tlds` — refreshes the bundled IANA TLD list. `--diff` shows added/removed entries without writing; `--force` overwrites unconditionally.
